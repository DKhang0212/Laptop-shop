...
class SiteController {
    ...
    onEditUserInfoHandler (req, res) {
        const {name, gender, doB, address, email, phone, username} = req.body;
        db.query('UPDATE users SET name = ?, gender = ?, doB = ?, address = ?, email = ?, phone = ? WHERE username = ?', [name, gender, doB, address, email, phone, username], (err, results) => {
            if (err) {
                console.log(err)
                return res.redirect('/user/profile?editInfoSuccess=false');
            };
            if (results) return res.redirect('/user/profile?editInfoSuccess=true');
        })
    }
